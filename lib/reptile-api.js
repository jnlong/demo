...
function setData(op) {
    ...
    fse.mkdirs(path, function(err) {
        ...
        getHtml(cg.link, fileName, true);
    });
}

function getHtml(link, fileName, isIndex) {
    request(link, function(error, res, body) {
        if (!error && res.statusCode == 200) {
            // 下载首页html文件
            let $ = cheerio.load(body);
            // 处理子页面
            isIndex && $(cg.menuFilter).each(function(i, elem) {

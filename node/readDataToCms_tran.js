...
function init() {
    ...
    for (var i in files) {
        ...
        log(data);
        var newData = encoding.convert(data, 'UTF-8', 'GBK');
        fs.writeFile(path.join(filePath + 'new\\', files[i]), newData);
    }
    log(resArr);
    // fs.writeFile('new.js', JSON.stringify(resArr));
}
